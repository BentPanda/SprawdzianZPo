...
def update_user(user_id):
    if user_id not in users or not request.json:
        return Response(status=400)
    user = users[user_id]
    user.update({k: request.json[k] for k in request.json if k in ['name', 'lastname']})
    return Response(status=204)

@app.route("/users/<int:user_id>", methods=['PUT'])
def replace_user(user_id):
    if user_id not in users or not request.json or 'name' not in request.json or 'lastname' not in request.json:
        return Response(status=400)
